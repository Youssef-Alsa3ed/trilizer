...
void Model::loadModel(string const &path)
{
    ...
    TRACELOG("Meshes Count: " + std::to_string(meshes.size()));
}
...
void Model::processMesh(aiMesh *mesh, const aiScene *scene)
{
    ...
    TriMat trimat{128.0f, std::move(textures)};
    TriMesh trimesh{vertices, indices};

    meshes.push_back(std::move(trimesh));
    mats.push_back(std::move(trimat));
}
...
vector<std::shared_ptr<Texture>> Model::loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName)
{
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
        ...
        {
            textures.push_back(loadedTextures.at(path));
            continue;
        }

        TRACELOG("Loading texture: " + path);

        std::shared_ptr<Texture> texture = std::make_shared<Texture>();
        texture->LoadAsync(path, loadTexturesFlipped);
...
Model::~Model()
{
    for(auto& pair : loadedTextures){
        CORELOG("Freeing Textures: " + pair.first);
