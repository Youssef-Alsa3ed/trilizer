...
struct Mesh {
  ...
  Mesh(const float *vertices, unsigned int vertCount,
...
void AppInit() {
  lightingshader =
      std::make_unique<Shader>("../../Assets/Shaders/lightshader.glsl");
  lightSourceShader =
      std::make_unique<Shader>("../../Assets/Shaders/lightsource.glsl");
  singlecolorshader =
      std::make_unique<Shader>("../../Assets/Shaders/outlineshader.glsl");

  cube = std::make_unique<Model>("../../Assets/Models/backpack/backpack.obj",
                                 true);
  cube2 = std::make_unique<Model>(
      "../../Assets/Models/companion_cube/scene.gltf", false);
  ...
  ImGui_ImplGlfw_InitForOpenGL(
      Application::GetInstance()->GetWindow().GetNativeWindow(), true);
...
void AppUpdate(float deltaTime) {
  ...
  LightManager::Get().lights[0].diffuse =
      vec3(clear_color.x, clear_color.y, clear_color.z);
  ...
  for (size_t i = 1; i < LightManager::Get().lights.size(); ++i) {
    ...
    LightManager::Get().lights[i].position =
        vec3(sinf(time * (0.5f + offset * 0.1f)) * (3.0f + offset),
  ...
  mat4 projection = glm::perspective(
      glm::radians(45.0f),
      Application::GetInstance()->GetWindow().GetAspectRatio(), 0.1f, 100.0f);
  ...
  for (size_t i = 1; i < LightManager::Get().lights.size(); i++) {
    auto model =
        glm::translate(mat4(1.0f), LightManager::Get().lights[i].position);
    model = glm::scale(model, vec3(0.3f));
    lightSourceShader->SetMat4("model", model);
    lightSourceShader->SetVec3("lightColor",
                               LightManager::Get().lights[i].diffuse);
...
bool ClientKeyCallBack(KeyPressEvent &e) { return true; }
